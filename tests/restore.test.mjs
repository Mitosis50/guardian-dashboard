...
for (const fileName of ['landing.html', 'index-landing.html']) {
  ...
  test(`${fileName} positions pricing around entry, recovery premium, and founder fair use`, () => {
    ...
    assert.ok(html.includes('Daily encrypted .md config snapshots'))
    assert.ok(html.includes('7-day version history'))
    assert.ok(html.includes('Self-serve restore'))
    assert.ok(html.includes('<div class="price">$19/mo</div>'))
    assert.ok(html.includes('Security + recovery premium'))
    assert.ok(html.includes('Snapshots every 6 hours'))
    assert.ok(html.includes('30-day version history'))
